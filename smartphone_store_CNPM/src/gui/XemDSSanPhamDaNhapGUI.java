...
public class XemDSSanPhamDaNhapGUI extends JDialog implements ActionListener {
    ...
    private CTSanPhamBUS ctSanPhamBUS = new CTSanPhamBUS();

    
    public XemDSSanPhamDaNhapGUI() {
        this.setModal(true);
        this.init();
    }
    ...
    public void loadCTSP() {
        if (ctSanPhamBUS.getCtspList() == null) {
            ctSanPhamBUS.list();
        }
        ArrayList<CTSanPhamDTO> ctspList = ctSanPhamBUS.getCtspList();
       modelBH.setRowCount(0);
        reloadCTSP(ctspList);
    }
    
    public void reloadCTSP(ArrayList<CTSanPhamDTO> ctspList) {
       modelBH.setRowCount(0);
        for (CTSanPhamDTO ctsp : ctspList) {           
               modelBH.addRow(new Object[]{
                    ctsp.getIdSanPham(), ctsp.getSerial()
                });           
        }
    }
