...
public class BanHangGUI extends JPanel implements ActionListener {
    ...
    private JButton btnTaoHoaDon, btnXoaSanPham, btnThemSanPham, btnChonKhachHang, btnChonKhuyenMai;
    ...
    public JPanel createPnInfor() {
        ...
        loadHD();
        String[] thuoc_tinh = {"Mã hóa đơn", "Mã khách hàng", "Khuyến Mãi", "Mã nhân viên", "Ngày"};
        ...
        this.arrTfInfor.get(3).setText(this.user.getIdUser());
        this.arrTfInfor.get(4).setText(LocalDate.now()+"");
        ...
        this.arrTfInfor.get(2).setPreferredSize(new Dimension(100, 30));
        this.btnChonKhuyenMai = new JButton("...");
        this.btnChonKhuyenMai.setPreferredSize(new Dimension(25, 25));
        this.btnChonKhuyenMai.setBackground(color3);
        this.btnChonKhuyenMai.setFont(font_infor);
        this.btnChonKhuyenMai.setForeground(color1);
        this.btnChonKhuyenMai.addActionListener(this);
        this.arrPnInfor.get(2).add(this.btnChonKhuyenMai);
        ...
        this.btnTaoHoaDon = new JButton("Xuất hóa đơn");
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        else if (e.getSource().equals(this.btnChonKhuyenMai)) {
            ChonKhuyenMai result = new ChonKhuyenMai();
            arrTfInfor.get(2).setText(result.getIdKM());
        }
    ...
    public void taoHoaDon() throws IOException {
        ...
        String id_nv = this.arrTfInfor.get(3).getText();
        LocalDate ngay_mua = LocalDate.now();
        int tt;
        String km = this.arrTfInfor.get(2).getText();
        if(km.equals("No")) tt = Integer.parseInt(this.lbTongTien.getText());
        else{
            tt = Integer.parseInt(this.lbTongTien.getText());
            if (kmBUS.getKmList() == null) {
                kmBUS.list();
            }
            ArrayList<KhuyenMaiDTO> kmList = kmBUS.getKmList();
            for(KhuyenMaiDTO Km : kmList){
                if(Km.getId_km().equals(km)){
                    tt = tt - Km.getTiLeGiamGia();
                }
            }
        }
        if (!id_kh.contains("KH")) {
            JOptionPane.showMessageDialog(null, "Chọn khách hàng!");
            return;
        }
        HoaDonDTO hd = new HoaDonDTO(id_hd, id_kh, km,id_nv,  ngay_mua, tt);
        ...
        for (CTHoaDonDTO cthd : arrCTHD) {
            for (int i = 0; i < cthd.getSoLuong(); i++) {
                ...
                BaoHanhDTO bh = new BaoHanhDTO(serial,cthd.getTenSanPham(),id_hd,id_kh,   ngay_mua, ngay_het_han);
