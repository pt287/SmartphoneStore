...
public class ThongKeGUI extends JPanel {
    ...
    public JPanel createPnFilter() {
        ...
        String[] tieu_chi = {
            "Tổng doanh số", "Doanh số nhập", "Doanh số bán", "Top 3 sản phẩm bán chạy", "Top 5 sản phẩm bán chạy","Doanh thu bảo hành"
        };
        ...
        // Nút lọc 
        JButton btn_loc = new JButton("Thống kê");
    ...
    public void createChart(LocalDate dateBegin, LocalDate dateEnd) {
        ...
        switch(tieu_chi) {
            ...
            case "Top 5 sản phẩm bán chạy":
                ...
                break;
            case "Doanh thu bảo hành":
                switch (loc_theo) {
                    case "Tháng":
                        pnChart.removeAll();
                        pnChart.add(chartDoanhThuBaoHanhThang(dateBegin, dateEnd));
                        pnChart.repaint();
                        pnChart.validate();
                        break;
                    case "Năm":
                        pnChart.removeAll();
                        pnChart.add(chartDoanhThuBaoHanhNam(dateBegin, dateEnd));
                        pnChart.repaint();
                        pnChart.validate();
                        break;
                }
    ...
    public JPanel chartDoanhThuBaoHanhThang(LocalDate dateBegin, LocalDate dateEnd) {
        JPanel pn_chart = new JPanel(new FlowLayout(1, 10, 10));
        pn_chart.setPreferredSize(new Dimension(this.width, 370));
        ...
        DefaultCategoryDataset dataset = new DefaultCategoryDataset();  
        for (LocalDate date = dateBegin; date.isBefore(dateEnd); date = date.plusMonths(1)) {
            dataset.addValue(Double.valueOf(ctBaoHanhBUS.tinhTienNhap(date, date.plusMonths(1))), "Tiền bảo hành", date.getMonth()+ "-" +date.getYear());
        }
    ...
    public JPanel chartDoanhThuBaoHanhNam(LocalDate dateBegin, LocalDate dateEnd) {
        JPanel pn_chart = new JPanel(new FlowLayout(1, 10, 10));
        pn_chart.setPreferredSize(new Dimension(this.width, 370));
        ...
        DefaultCategoryDataset dataset = new DefaultCategoryDataset();  
        for (LocalDate date = dateBegin; date.isBefore(dateEnd); date = date.plusYears(1)) {
            dataset.addValue(Double.valueOf(ctBaoHanhBUS.tinhTienNhap(date, date.plusYears(1))), "Tiền bảo hành", date.getYear()+"");
        }
