...
public class QuyenGUI extends JPanel {
    ...
    public JPanel createPnInfor() {
        ...
        result.setPreferredSize(new Dimension(this.width, 310));
        
        JPanel pn_infor = new JPanel(new BorderLayout());
        pn_infor.setPreferredSize(new Dimension(this.width - 100, 280));
        ...
        pn_desc.setPreferredSize(new Dimension(350, 320));
        ...
        String[] quyen = {
            "Nhân viên", "Khách hàng", "Nhà cung cấp","Khuyến mãi", "Sản phẩm", "Quyền", "Bảo hành", "Bán hàng", "Nhập hàng", "Hóa đơn", "Phiếu nhập" 
        };
                ...
                if (isEditing) { // đang trong chế độ sửa
                    ...
                    if (confirmed == 0) { // xác nhận sửa
                        ...
                        if (ten.isEmpty() || ten.isBlank()) {
                            JOptionPane.showMessageDialog(null, "Điền tên quyền!");
                            return;
                        }
...
//            int id_cn = Integer.parseInt(arr_cn.get(i).substring(0, 2)+""); // Lấy 2 ký tự đầu tiên
            arr_key.add(id_cn);
        }
...
//                int id_loai = Integer.parseInt(arr_cn.get(i).substring(1, 3));
                if (key.equals(id_cn)) {
